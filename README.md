...
### 1.5. フロントエンドAPI接続設定 (Optional)

フロントエンド側が接続するバックエンドのAPI URL（デフォルトは `http://localhost:8080`）を変更する場合は、`client/.env` ファイルを作成して以下のように指定してください。
...
cd client
npm install
# (任意) 通信先の設定: client/.env で VITE_API_URL を指定
npm run tauri dev
```
